...
router.get("/:slug/comments", auth.authenticationOptional, async(req, res, next) => {
...
router.get("/feed", auth.verifyToken, async(req, res, next) => {
    ...
    try {
        let articlesArr = await Article.find({ author: { $in: req.user.followings } }).sort({ createdAt: 1 }).limit(limit).skip(offset).populate('author').exec();
...
router.get("/", auth.authenticationOptional, async(req, res, next) => {
    ...
    try {
        let articlesArr = await Article.find(query).sort({ createdAt: 1 }).populate("author").limit(limit).skip(offset).exec();
        articlesArr = articlesArr.map((article) => { return article.getArticleFormat(req.user) });
...
router.get("/:slug", auth.authenticationOptional, async(req, res, next) => {
    ...
    try {
        ...
        if (article) {
            return res.status(200).json({ singleArticle: article.getArticleFormat(req.user) });
        } else {
...
router.post("/", async(req, res, next) => {
    try {
        ...
        let updatedUser = await User.findByIdAndUpdate(req.user.id, { $push: { articles: article.id } }, { new: true });
        return res.status(200).json({ article: article.getArticleFormat(req.user) });
    } catch (error) {
        next(error);
    }
});
    ...
    try {
        if (article) {
            if (req.user.id == article.author) {
                ...
                return res.status(200).json({ updatedArticle: updatedArticle.getArticleFormat(req.user) });
            } else {
    ...
    try {
        if (article) {
            if (req.user.id == article.author) {
                ...
                let updatedUser = await User.findByIdAndUpdate(req.user.id, { $pull: { articles: article._id } }, { new: true });
                return res.status(200).json({ deletedArticle: deletedArticle.getArticleFormat(req.user) });
            } else {
...
router.post("/:slug/comments", async(req, res, next) => {
    ...
    if (article) {
        ...
        return res.status(400).json({ error: `not able to find any article with this name ${slug}` });
    }
    try {
    ...
    try {
        if (comment) {
            if (comment.author == userId) {
                ...
                return res.status(422).json({ error: 'you cannot deleted comments of others' });
            }
        } else {
...
router.post("/:slug/favourite", async(req, res, next) => {
    ...
    try {
        if (article) {
            if ((await article).favouritedBy.includes(userId)) {
                return res.json({ msg: "Already added in the favourites by you." });
            } else {
                let updatedArticle = await Article.findByIdAndUpdate(article.id, { $push: { favouritedBy: userId } });
                return res.status(200).json({ success_msg: "favourited by you." });
            }
        } else {
    ...
    try {
        if (article) {
            if (await article.favouritedBy.includes(userId)) {
                ...
                return res.status(200).json({ success_msg: "unfavourited by you." });
            } else {
                return res.json({ msg: "not favourited by you." });
            }
        } else {
